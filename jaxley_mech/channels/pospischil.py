...
class Na(Channel):
    ...
    def update_states(
        ...
        new_m = solve_gate_exponential(m, delta_t, *self.m_gate(voltage, params["vt"]))
        new_h = solve_gate_exponential(h, delta_t, *self.h_gate(voltage, params["vt"]))
    ...
    def compute_current(
        ...
        current = gNa * (voltage - params["eNa"])
        return current
    ...
    def init_state(
        ...
        prefix = self._name
        alpha_m, beta_m = self.m_gate(voltage, params["vt"])
        alpha_h, beta_h = self.h_gate(voltage, params["vt"])
        return {
...
class K(Channel):
    ...
    def update_states(
        ...
        new_n = solve_gate_exponential(n, delta_t, *self.n_gate(voltage, params["vt"]))
    ...
    def init_state(
        ...
        """Initialize the state such at fixed point of gate dynamics."""
        prefix = self._name
        alpha_n, beta_n = self.n_gate(voltage, params["vt"])
...
class Km(Channel):
    ...
    def update_states(
        ...
        new_p = exponential_euler(
            p, delta_t, *self.p_gate(voltage, params[f"{prefix}_taumax"])
        )
    ...
    def init_state(
        ...
        """Initialize the state such at fixed point of gate dynamics."""
        prefix = self._name
        alpha_p, beta_p = self.p_gate(voltage, params[f"{prefix}_taumax"])
...
class CaL(Channel):
    ...
    def compute_current(
        ...
        return gCaL * (voltage - params["eCa"])
    ...
    def init_state(
        ...
        """Initialize the state such at fixed point of gate dynamics."""
        prefix = self._name
        alpha_q, beta_q = self.q_gate(voltage)
        alpha_r, beta_r = self.r_gate(voltage)
        return {
...
class CaT(Channel):
    ...
    def update_states(
        ...
        new_u = exponential_euler(
            u, delta_t, *self.u_gate(voltage, params[f"{prefix}_vx"])
        )
    ...
    def compute_current(
        ...
        prefix = self._name
        u = states[f"{prefix}_u"]
        s_inf = 1.0 / (1.0 + save_exp(-(voltage + params[f"{prefix}_vx"] + 57.0) / 6.2))

        gCaT = params[f"{prefix}_gCaT"] * (s_inf**2) * u  # S/cm^2

        return gCaT * (voltage - params["eCa"])
    ...
    def init_state(
        ...
        """Initialize the state such at fixed point of gate dynamics."""
        prefix = self._name
        alpha_u, beta_u = self.u_gate(voltage, params[f"{prefix}_vx"])
