...
class RibbonSynapse2(Synapse):
    ...
    def update_states(self, u, delta_t, pre_voltage, post_voltage, params):
        ...
        p_d_t = 1.0 / (1.0 + save_exp(-k * (pre_voltage - V_half)))
        
        # Glutamate release
        e_t = params[f"{name}_e_max"] * p_d_t * u[f"{name}_RRP"] / params[f"{name}_RRP_max"]
        # Rate of RP --> IP, movement to the ribbon
        r_t = params[f"{name}_r_max"] * (1 - u[f"{name}_IP"] / params[f"{name}_IP_max"]) * u[f"{name}_RP"] / params[f"{name}_RP_max"] 
        # Rate of IP --> RRP, movement to the dock
        i_t = params[f"{name}_i_max"] * (1 - u[f"{name}_RRP"] / params[f"{name}_RRP_max"]) * u[f"{name}_IP"] / params[f"{name}_IP_max"]
